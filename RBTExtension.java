...
public class RBTExtension<T extends Comparable<T>> extends RedBlackTree<T> {
   ...
   */
  private Node<T> deleteHelper(T data) {
    ...
    if (returnNode == root && returnNode.leftChild == null && returnNode.rightChild == null) {
      root = null;
      return returnNode;
    }
  ...
  public Node<T> search(Node<T> node, T data) {
    ...
    returnNode = search(node.leftChild, data);
    if (returnNode != null) {
      return returnNode;
    }
    ...
    returnNode = search(node.rightChild, data);
    if (returnNode != null) {
      return returnNode;
    }
    return null;
  ...
  public void searchPrint(Node<T> node) {
    ...
    if (node.parent != null) {
      System.out.println("Parent: " + node.parent.data.toString() + " | Color: " + node.isBlack);
    }
