 ...
 * search tree. Insert method used to build a binary search tree, and toString method to display the
 * level order (breadth first) traversal of values in that tree.
 */
public class RedBlackTree<T extends Comparable<T>> {

  /**
   * This class represents a single node holding a value within a binary tree the parent, left, and
   * right child references are maintained correctly.
   */
  ...
  /**
   * This private method enforces the RBT properties after a new node has been added.
   * 
   * It only works with red nodes that are children of red nodes because all new nodes are red, and
   * if their parent is black, then no property is violated.
   * 
  ...
  private void enforceRBTreePropertiesAfterInsert(Node<T> child) {
    ...
    if (siblingOfParent == null || siblingOfParent.isBlack)
      ...
      if ((child.isLeftChild() && parent.isLeftChild())
          || (!child.isLeftChild() && !parent.isLeftChild())) {
    ...
    // Case 2 - Parent's sibling is red
      ...
      // We recolor to restore properties and call this method recursively to ensure
